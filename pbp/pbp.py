...
class PseudoBooleanPolynomials:
    ...
    def combinations_without_repeat(self, a):
        n = len(a)
        out = np.empty((n,n-1,2),dtype=a.dtype)
        out[:,:,0] = np.broadcast_to(a[:,None], (n, n-1))
        out.shape = (n-1,n,2)
        out[:,:,1] = self.onecold(a)
        out.shape = (-1,2)

        return out

    def get_adjacent_list(self, yy :np.ndarray, p) -> np.ndarray:
        self.anti_chains = self.shrink_y(yy.copy())
        self.yy = self.anti_chains.astype(np.uint64).ravel()
        self.yy = self.yy[self.yy.nonzero()[0]]

        self.yy.sort()
        # res = np.zeros((self.yy.shape[0], self.yy.shape[0]), np.uint8)

        adj_list = []

        indices = np.arange(self.yy.shape[0])

        start_time = timeit.default_timer()
        res_x = self.f(*np.meshgrid(indices, indices, sparse=True, copy=False))
        res_x = np.tril(res_x, k=-1)
        chains = np.argwhere(res_x == True)
        elapsed = timeit.default_timer() - start_time
        print("[INFO] res x using meshgrid {:.4f}s".format(elapsed))

        ys, xs = chains.T

        print("xs, ys", np.stack([xs, ys]))

        adj_list = np.stack([self.yy[xs], self.yy[ys]]).T
    ...
    def get_adjacent_list_dec_21(self, yy :np.ndarray) -> np.ndarray:
        self.anti_chains = self.shrink_y(yy.copy())
        self.yy = self.anti_chains.astype(np.uint64).ravel()
        self.yy = self.yy[self.yy.nonzero()[0]]
        ...
        for i in tqdm.tqdm(range(self.yy.shape[0])):
            for j in range(i):
                if not (self.yy[j] == self.yy[i]):
                    r = self.yy[j] ^ self.yy[i]
                    r = (r & r - 1) == 0
                    if r:
                        res[j, i] = 1
                    del r
    ...
    def hasse_digram(self, bin_adj_list, out_path="./p.svg", show_diag=False, cb=None) -> None:
        ...
        for bin_rows in bin_adj_list:
            ...
            for bin_str in bin_rows:
                nodes.append(self.get_monomial_str(bin_str))
        ...
        try:
            ...
            png_file = "{}.png".format(".".join(out_path.split(".")[:-1]))
            cairosvg.svg2png(
                        url=out_path, write_to=png_file)
            ...
            if cb is not None:
                ...
                png_as_text = "data:image/png;base64,{}".format(img_str.decode('ascii'))
    ...
    # @staticmethod
    def get_monomial_str(self, bin_row: np.ndarray) -> np.ndarray:
        jth = np.argwhere(bin_row)
        res = ''.join((jth+1).astype(np.str_).ravel()) 

        return res

    def visualize_monomials(self, y: np.ndarray)-> np.ndarray:
        yy = y.copy()

        stacks = []
        for _ in range(self.byte_size//8):
            yy = yy + (1 << 8)

            un_packed = np.unpackbits(yy.astype(np.uint8), axis=1, bitorder=self.bit_order)
        ...
        if len(yy.shape) > 1:
            stacks = np.reshape(stacks, (-1, yy.shape[1], self.byte_size))
        else:
            stacks = np.reshape(stacks, (-1, self.byte_size))

        return stacks

    @staticmethod
    def reduce_similar_monomials(y: np.ndarray, delta_c: np.ndarray) -> str:
        kv = {}
        flat_d_c = delta_c.ravel()

        for i,k in enumerate(y.ravel()):
            if kv.get(k, None) is None:
                kv[k] = 0
            kv[k] += flat_d_c[i]

        return ["{}{}".format(v, k) for k, v in kv.items() if v]

    def truncation_str(self, reduced_pbp: str, p: int) -> str:
        truncated = []
        m = self.c.shape[0]

        for i in tqdm.tqdm(range(len(reduced_pbp))):
            if len(reduced_pbp[i].split("y")) <= m - p + 1:
                truncated.append(reduced_pbp[i])
